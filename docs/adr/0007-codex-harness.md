...
Luna 在 Context 的 `response_language` 字段传递当前生效的界面语言（已解析“跟随系统”）：`zh`、`zh_hant`、`en`、`ja`、`pt_br`、`es`、`ru`、`ko`、`vi`。此字段与终端/脚本的命令语言独立。Kael 仅将白名单中的语言值映射为固定语言名称，附在本轮输入末尾；开发者提示词中的语言规则保持不变。进度、解释、提问和最终回复默认使用该语言；用户明确指定回复语言或翻译目标时遵循用户要求。命令、SQL、标识符、路径、资源名及引用的错误/日志保持原文。字段缺失或不支持时跟随用户最新问题的语言，无法判断时使用英语。

Luna 创建或更新 Context 时携带语言，切换界面语言后在下一轮 Run 前刷新 Context，保留上次上传的其他上下文。语言变化不改变 thread 复用签名：新语言随本轮输入追加，保留已有请求前缀和会话，已开始的 Run 沿用其冻结快照。此前缀稳定性为模型缓存提供复用条件，实际命中仍取决于模型服务的缓存策略、有效期和路由，应查看真实请求的 `cached_tokens`，不能用本地 fixture 的模拟 usage 推断命中率。
