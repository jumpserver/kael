...
已接受，2026-09-03；其中默认 Store 的实现选择先后由 [ADR 0004](./0004-jsonl-store-and-event-protocol.md) 和 [ADR 0006](./0006-core-backed-runtime-journal.md) 替代。本文关于 Store port、Kael 不直连数据库、组件身份和模型配置的边界继续有效。
...
- 当前 Store adapter 的持久化与恢复语义见 ADR 0006。Runtime 仍不感知 adapter 是内存、本地 JSONL 或 Core-backed Journal。
- 当前不提供多实例共享或 durable background Run。Core-backed Journal 可恢复历史，但不能恢复浏览器执行连接；bootstrap 默认返回 `storage.kind=core`、`storage.durable=true` 和 `background=false`。
...
Kael 可以像 Koko 一样由 Core 注册和集中下发模型配置，且本地启动不要求 DSN。Conversation 历史由 ADR 0006 的 Core-backed Journal 保留；浏览器能力和 SSE stream 仍是进程绑定的。任何要求后台运行、未决 Approval 续接或多实例无状态切换的部署，都必须另外提供分布式 ownership 和相应的安全恢复协议。
