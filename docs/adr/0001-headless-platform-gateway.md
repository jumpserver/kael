...
ADR 0002 已覆盖本文对后台 Run、持久 Approval 和 Kael 自有数据库的实现假设，ADR 0006 又将默认 Runtime Store 从本地 JSONL 改为 Core-backed Journal。Headless Gateway 当前只用于前台 service capability；保留隔离 Provider 和 Core 最终 RBAC，业务鉴权改为沿用用户 Cookie（或已有 Authorization）。
...
- service Run 固化创建时的 superuser/org-admin flags 和权限列表，供异步模型轮次维持相同的搜索可见性；Core 使用用户凭据后仍按该 user/org 的实时状态和 DRF RBAC 复核，因此旧快照不能在权限撤销后授权执行。
- 每次 Core 业务请求沿用经过身份认证、Origin/CSRF 检查的用户 Cookie（或已有 Authorization）及组织头，写请求同时携带 CSRF token。凭据仅按 Run 保存在进程内存中，不写入 Journal、审计或模型输入；执行结束、取消或清理终态 Run 后释放，恢复未执行 Run 时重新绑定当前请求凭据。Core 继续检查会话有效性、用户状态、组织边界和最终 RBAC；会话失效后调用失败，不自动提升为组件身份。
...
Lina 未传 Profile 时默认选择 `general`，因此 Platform Gateway 是 Kael 的必备启动依赖，不再是可降级功能。`PLATFORM_GATEWAY_ENABLED` 默认且必须为 `true`；显式关闭或 Registry 初始化失败都会阻止 Kael 监听端口。业务调用无需额外共享密钥；组件 AccessKey 继续用于读取 OpenAPI 和 Runtime Store。
