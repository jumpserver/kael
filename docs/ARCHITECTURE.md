...
| 状态 | 目标架构基线 |
| 日期 | 2026-09-04 |
| 适用范围 | Kael AI Runtime、Luna AI Panel 及其能力适配边界 |
| 当前实现状态 | Kael/Luna 逻辑迁移已实现；Kael 使用 Core component + 本地 JSONL Store，边界见 ADR 0002/0004 |
...
- [Platform AI 现状契约与迁移边界](./platform-ai-compatibility.md) 记录旧 Platform AI 的真实契约、兼容面和迁移阻塞。
...
Kael 是注册到 Core 的 Terminal component，也是通用推理与编排 Runtime；AI Panel 是 Agent Host。模型配置来自 Core TerminalConfig。Koko、Chen 和本地 UI/Script 通过 Luna 的 panel binding 提供；Core API 通过 ADR 0001 的 service binding 提供，并由通用 Capability Broker 与 Runtime core 隔离。
...
5. 所有 AI 业务接口只使用 `/kael/api/v1`，不提供其它业务根路径或按对话类型拆分的入口。
6. Conversation 与 Message 历史可通过当前 JSONL Store 跨 Kael 重启存在；PanelSession、Registration 和 executor 连接不跨进程恢复。每次 Run 固定本次执行环境和能力快照。
...
命令流由 Luna 通过 HTTP 提交给 Kael；Kael 按 PanelDelivery audience 通过 SSE 投递状态，panel-scoped 文本增量、ToolCall 和 Approval 回到原 Panel；可共享的脱敏终态可以投影给同一 Conversation 的其它授权 Panel。panel-scoped ToolResult 由 Luna 通过 HTTP 回传。

service binding 已按 ADR 0001 启用：Capability Broker 把调用路由给 Headless Provider，该执行路径不使用 Panel SSE，状态和 Approval 仍通过统一 Event/PanelDelivery 投影。按 ADR 0004，历史和 DomainEvent 可恢复，但未决 Approval、活动 ToolCall 和 Panel capability 不跨 Kael 重启续接。
...
- Lina、Core、Magnus、Lion 和 Koko 代码不在本期修改范围；
...
| Conversation、Message、Artifact 元数据 | Kael Store port；当前为本地 JSONL adapter | Luna 本地状态只是视图缓存；历史可跨同一数据目录的 Kael 重启 |
...
| Legacy Platform 数据 | 完成迁移前仍由旧 Platform 存储负责 | 不能让两个系统同时写同一对象 |
...
若为了完整 Platform 兼容引入 Headless Platform Gateway，它必须是独立 Adapter 或独立部署单元，不能反向污染 Runtime 核心。是否启用该 Gateway 仍是实施前决策，见 12.3 节和第 19 节。
...
| background | 任意 | 当前非法；外部 durable Store adapter 完成前返回 `background_requires_durable_store` |
...
- 当前所有 `execution_mode=background` 组合都非法；具备分布式 ownership 与安全工具恢复的外部 Store adapter 完成后才能重新启用；
...
依赖 Panel 本地 executor 的长 ToolCall 仍受 PanelSession lease 约束。需要浏览器关闭后继续执行的 Platform Agent Loop，必须使用可信 Headless Provider 或保留旧服务，不能假装 Luna 仍在线。
...
- 浏览器始终通过 Luna 的 site-prefix helper 构造地址；
...
| Artifact/STT | `/kael/api/v1/artifacts`、`/kael/api/v1/artifacts/{id}`、`/kael/api/v1/transcriptions` |
...
| Platform 管理 | `/kael/api/v1/admin/platform-registry/refresh`、`/kael/api/v1/admin/stats`、`/kael/api/v1/admin/audit/conversations` |
...
- PanelDelivery 是具体 PanelSession 的重放真值；JSONL 可保留它用于审计，但 PanelSession 重启后失效，新的 Panel 不能沿用旧 cursor。
...
| 类型 | 内容 | 当前要求 |
|---|---|---|
| Runtime state | Conversation、Message、Run、ToolCall、ToolResult、Approval、DomainEvent、PanelDelivery、审计索引 | 仅通过 Store port；当前写入本地 JSONL journal |
| Artifact state | 图片、文件和派生内容 | 所有权、大小限制、加密、保留和删除 |
...
Kael 当前不连接数据库，也不包含 ORM 或 schema migration。当前 adapter 使用 `data/store/runtime.jsonl` 保存事务状态，并把 DomainEvent 写入 `data/events/<conversation-id>.jsonl`。未来可以实现外部 Store adapter，但 Runtime、领域对象、HTTP 和 Event 契约不得感知具体存储产品。
...
- 不同 Kael 实例不共享本地 JSONL 文件，不能并发写同一个数据目录；
- 当前生产入口必须使用单写实例与实例粘性；滚动或故障切换会中断活动执行，但挂载同一安全数据目录时可恢复历史；
- 具备分布式协调的外部 Store adapter 上线前，禁止宣称无状态横向扩容、跨实例 Panel 恢复或后台执行。

## 12. Platform AI

### 12.1 目标范围

Platform AI 纳入 Kael 的目标能力，而不是只迁移 Luna 的文本聊天子集。兼容基线包括：

- Conversation、Message、Artifact 和历史；
- stream、background、cancel 和恢复；
- branch、regenerate；
- Web Search、STT；
- Approval、result card/activity；
- quota、cleanup、stats 和脱敏 audit；
- general、management、asset、session_audit、ops Assistant/Profile。

当前 `general` 是 chat-only；`management` 才包含管理员范围内的动态 Core 能力。旧文档中的 Scheduled Report 没有对应实现，不属于迁移基线。
...
现有 `management` 的动态 OpenAPI 是必须明确处置的迁移兼容能力。在稳定语义工具完全覆盖前，它只能保留在旧 Platform 服务或隔离的 Headless Platform Gateway 中；Gateway 仍必须以 operation ID、可信 Method/URL 构造、allowlist、schema、敏感路径和二次 RBAC 限制模型，不能退化为任意 HTTP 工具。删除这项动态能力属于产品范围缩减，不能称为完整等价迁移。
...
长期推荐边界是：Runtime core 保持业务无关，Platform Capability 由 Lina 或独立可信 Gateway 提供。若“本期完整等价迁移”是硬发布条件，可在 Kael 仓库增加隔离的 Headless Platform Gateway，但必须：
...
- 长期可承担后台 Tool、持久 Approval、动态 Registry 和执行审计；当前按 ADR 0004 只启用前台执行，历史状态写入 JSONL，活动能力仍为进程绑定；
...
本期已通过 [ADR 0001](./adr/0001-headless-platform-gateway.md) 选择过渡 Gateway，并引入通用 CapabilityProvider/ExecutionBinding。Legacy 数据仍按该 ADR 由旧服务只读保留；生产上线状态以 [迁移实施状态](./MIGRATION_STATUS.md) 为准。

### 12.4 Legacy 数据

旧 Conversation、Message、附件、Run、Approval 和 Audit 必须选择：

- 迁移到 Kael；
- 由旧服务只读保留；
- 通过明确的只读查询层访问。

不能让 Kael 与旧 Platform 服务同时成为同一对象的写权威，也不能静默丢失历史。详细字段和切流条件见 Platform AI 专题文档。
...
Kael 当前将 API、Worker 和本地 JSONL Store 部署在同一二进制中。需要独立扩缩容前，必须先实现外部 Store adapter 和跨实例路由。
...
Kael component -------- registration / AccessKey / TerminalConfig / heartbeat --------> Core
```
...
- readiness 检查组件注册、TerminalConfig 模型配置、JSONL Store 和 Worker 接单能力；
...
- 旧路径下线必须满足新流量切换、在途 Run 排空、Approval 处理、历史可读和回滚验证。
- Koko agentd 删除、Lina 切换、Core Gateway 和 Legacy 数据处理分别设置发布 Gate。
...
- 本期修改或删除 Koko、Chen、Core、Lina、Magnus 或 Lion；
...
以下事项已由 [ADR 0001](./adr/0001-headless-platform-gateway.md)、[ADR 0002](./adr/0002-core-component-and-store-port.md)、[ADR 0003](./adr/0003-flat-config-and-ablation.md)、[ADR 0004](./adr/0004-jsonl-store-and-event-protocol.md) 和 [迁移实施状态](./MIGRATION_STATUS.md) 冻结：

1. Platform AI 使用严格目标架构、过渡 Headless Platform Gateway，还是阶段性保留旧服务；
2. 动态 Core OpenAPI 的前台承接方；后台 Core Tool 和跨重启 Approval 当前禁用；
3. 旧 Platform 数据迁移、只读保留或查询层方案；
...
6. 当前使用本地 JSONL Store，且不读取 Koko 历史 event；未来外部持久化仍只能通过 Store port adapter，Artifact 继续使用私有目录；
7. 当前使用 session stickiness，不支持跨实例状态恢复；
8. Event/cursor、Artifact、审计、幂等键和历史的保留期限；
9. worker、lease、retention、Run timeout 和 payload 限制使用 Runtime 安全默认值，不扩大部署配置面；
10. 生产网关、CSRF、Origin、SSE、探针和 metrics 的部署策略；
11. 灰度维度、观测阈值、回滚条件和旧服务退出时间。
