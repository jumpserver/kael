...
Lina 在用户显式选中页面引用后，以 Message `data` Part 持久化最小业务上下文，并在 Run 前写入版本化 ContextSnapshot。重新生成和编辑分支沿用原问题快照，路由/组织切换不会悄悄替换历史上下文。允许字段和选择范围由客户端白名单限定，所有资源查询/操作仍按服务端实时权限执行。

现有结果卡增加由成功 Core operation 与明确资源 ID 生成的受控详情入口。Message/Run 的可选 `failure` 对象及终态事件显示失败阶段、确认已完成/结果未确认步骤与下一步；不把中断写操作推断为未执行，不以一键重试重复已完成或结果未确认的写入。旧历史没有该字段时，界面只根据已有执行记录提供保守说明。
