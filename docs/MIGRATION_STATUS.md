...
Runtime 仍只依赖 `ports.Store`/`ports.Tx` 抽象。默认 adapter 通过组件签名调用 JumpServer Core Runtime Store API，以 CAS 只追加 Conversation、用户问题、终态回答、结果卡片和关联 Artifact 的历史 delta；Run、DomainEvent、PanelSession、Registration、Tool/Approval 和 SSE 状态不跨 Kael 节点恢复。Artifact 原始字节不在 Journal 中，替换节点仍需挂载或迁移原 `data/artifacts` 卷。Terminal AI 固定使用独立本地 JSONL，存储位置按会话类型划分，不提供模式切换或 Core 故障时的自动 failover。
...
| Model | 完成 | 从 TerminalConfig 读取并在每次 Run 前刷新 `CHAT_AI_ENABLED/PROVIDER/BASE_URL/API_KEY/PROXY/MODEL`；禁用或未完成配置不阻止 Kael 启动，仅拒绝 Run，本地配置不保存模型 Secret |
| Store port | 完成 | Runtime 只依赖通用 Store/Tx 接口；普通会话历史由 Core 保存 opaque Journal，Terminal AI 使用本地 JSONL；Kael 不直接使用数据库 |
...
| Event/SSE | 完成 | Terminal AI 的 DomainEvent 随本地 Runtime Journal 持久化，其他会话事件保存在进程内；`message.delta` 约 50ms/1KiB 合并并在模型/工具/Run 边界 flush；PanelDelivery 使用 PanelSession 私有 sequence，Lina 直接消费其 dot 事件名，不经过 Legacy event adapter；旧 Panel cursor 不跨重启恢复；Terminal AI 本地 JSONL 另写 Conversation event archive |
...
- Lina 增加显式页面引用；支持的业务页提供当前页面标识，资产列表/详情可引用允许字段及最多 10 个选中资产。发送前写入已有 ContextSnapshot 接口，并随原问题保存引用以供重新生成/分支沿用；切换页面或组织会清除输入框引用，不能跨组织复用。
- 结果卡对有明确资源 ID、已核实路由且有权限的成功 Core 结果显示详情入口；不使用模型生成链接，也不将行号当资源 ID。
- 失败/取消提示显示阶段、已确认完成的步骤、结果未确认的步骤和下一步；模型配置、限流、超时、权限和执行连接错误使用稳定错误码及本地化提示。故障摘要随历史持久化；已完成或结果未确认的写操作先核对资源，避免快捷重试重复执行。
- 本轮不新增单元测试，生产构建与静态检查通过。真实浏览器已验证选中资产引用、只读查询、结果详情跳转、切页取消引用及详情页重新引用；错误卡片以明确标注的模拟回执验证限流重试、已完成变更和结果未确认场景，未通过实际写操作制造故障。联网搜索继续保持关闭。
...
- Core 正常运行不再生成周期性 snapshot，只追加精简历史 delta；升级首次加载旧版完整运行态时会用一次精简 snapshot 完成迁移。Terminal AI 写 `data/terminal/store/runtime.jsonl` 和 `data/terminal/events/*.jsonl`。
...
- `/kael/health/ready` 检查进程内 Store 已初始化、未关闭且未 poisoned，并检查当前持久化 adapter；对 Core 在 2 秒超时内发起带签名的 `runtime-store` 轻量探测并校验 receipt 与本地 revision 未分叉，同时检查 Terminal AI 本地 journal 仍可用。它不检查 Worker 或模型端点。`/kael/internal/metrics` 没有业务用户认证，必须由反向代理或网络 ACL 限制到监控网络。
...
- 发布前确认 Core 路由清单中 `/api/v1/chat-ai/` 只剩 `runtime-store/`，并清理旧开发分支留下的 AI 表/数据。回滚时先停止创建新 Kael Run，排空或取消在途 Run/Approval，再成对回滚相互匹配的 Lina/Luna/Kael 构建；不得回退到已删除的 Core ChatAI Runtime。Core Journal、Terminal AI 本地数据与 Artifact 卷必须保留。
